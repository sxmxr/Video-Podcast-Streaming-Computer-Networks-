//for compile:  gcc `pkg-config gtk+-3.0 --cflags` client.c -o client `pkg-config gtk+-3.0 --libs`
// 		./client 127.0.0.1
...
void clicked_button2(char* argv[])
{
    ...
    frame = gtk_frame_new ("");
    label = gtk_label_new ("Select your Mood");
  
    gtk_container_add (GTK_CONTAINER (frame), label);
    gtk_box_pack_start (GTK_BOX (vbox), frame, TRUE, TRUE, 0);    
    
    //Assigning labels to the 3 buttons corresponding to 3 stations
     btn = gtk_button_new_with_label("Reading Music ");
    btn1 = gtk_button_new_with_label("Sleeping Music ");
    btn2 = gtk_button_new_with_label("Trip Music ");
...
int main(int argc, char * argv[])
{
    ...
    btn2=gtk_button_new_with_label("Change The Mood");
    btn3=gtk_button_new_with_label("Stop");
