...
void *sendLiveStream(struct liveSenderArgumemnts * temp)
{
  ...
  strcpy(a, "/home/sxmxr/CNProject/M/Server");
  ...
  fp = NULL;
  fp = fopen("final.txt","r");
  system("ls -Art /home/sxmxr/CNProject/M/Server | tail -n 1 >> final.txt");
...
void *sendStreamThread(struct senderArguments * temp)
{
  ...
  {
  system("ffplay -autoexit -i ../Client/output.mp4");
  }
  i++;
  close(sArg.sStream);
}
...
void *sendDataThread(struct senderArguments * temp)
{
  ...
  if(sArg.stationNumber == 1)
  {
    picture = fopen("temp1.mp4", "r");
    if(picture==NULL)
    {
      printf("Error in opening temp1.mp4");
      exit(1);
    }   
    ...
    system("ffmpeg -i temp1.mp4 2>&1 | grep Duration | cut -d ' ' -f 4 | sed s/,// >> duration1.txt");
  }
  else if(sArg.stationNumber == 2)
  {
    picture = fopen("temp2.mp4", "r");   
    if(picture==NULL)
    {
    ...
    system("ffmpeg -i temp2.mp4 2>&1 | grep Duration | cut -d ' ' -f 4 | sed s/,// >> duration2.txt");
  }
